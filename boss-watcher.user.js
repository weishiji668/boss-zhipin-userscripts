...
// @updateURL    https://raw.githubusercontent.com/weishiji668/boss-zhipin-userscripts/main/boss-watcher.user.js
// @downloadURL  https://raw.githubusercontent.com/weishiji668/boss-zhipin-userscripts/main/boss-watcher.user.js
...
function onPanelClick(e){
  ...
  else if(act==='intel'){ alert('公司性质判定已移到独立脚本 boss-insight.user.js（岗位体检）。'); }
  ...
  else if(act==='aitest'||act==='setkey'||act==='clearkey'){ alert('AI 判定已拆到独立脚本 boss-insight.user.js（岗位体检）：油猴菜单「🤖 设置 AI 接口」里填服务商/地址/Key。'); }
...
function renderSettings(){
  ...
  html+='<div class="bw-note">「岗位体检」相关功能已拆到独立脚本 <b>boss-insight.user.js</b>，但它现在<b>只做聊天会话体检</b>（风险关键词扫 HR 发来的消息、本地隐藏、站内处置）。<br>原先的「岗位风险打分」和「AI 跟进建议」都已删除（用户口径：没用），所以本面板不会再显示风险分或 🤖 建议。设置入口在油猴菜单「⚙ 岗位体检设置」。</div>';
