...
const TEMPLATE_ROWS=[
  ...
  ,['关键词','AI判定','包含','中介','标记','否','（已停用）现在没有脚本在写「AI判定」数据，这条永远不命中；要按公司名排除请改用「公司名」+黑名单']
...
async function fetchPageDetails(){
  ...
  if(jdCooling()){ jdLastMsg='限流冷却中：还要 '+Math.ceil((jdCoolUntil-Date.now())/60000)+' 分钟（上次被站点限流）'; renderJdStat(); return; }
  ...
  if(!n){
    ...
    jdLastMsg=todo.length?('今日补取已达每日上限（'+(cfg.jdDaily||40)+' 次）'):(all?('本页岗位都已经取过（'+all+' 张卡都有正文）'):'没识别到职位卡片，没法补取');
    renderJdStat(); return;
  }
  jdBusy=true;
    ...
    for(let i=0;i<n;i++){
      ...
      if(r.ok){ ok++; jdStreak=0; schedule(); }   // 取到一条就先刷一次列表，不用等整批跑完
        ...
        if(r.cool||++jdStreak>=3){
          ...
          cooled=true;
          jdLastMsg='已停：连续 '+(r.cool?'被站点限流':jdStreak+' 次')+'（'+(r.why||'')+'），限流冷却 15 分钟';
          break;
        }
      }
...
function importRows(rows){
  // v1.3.5：支持「一表对一框」的新模板 —— 按表头识别这是哪张表，写进对应字段。
  // 老的两页模板（类型/字段/匹配/值/动作/启用/备注）继续兼容。
  if(!rows || !rows.length) return 0;
  const head=Object.keys(rows[0]||{}).join(',');
  const has=(k)=>new RegExp(k).test(head);
  let n=0;
  // ① 基础开关与薪资：表头 = 项目,填写值
  if(/项目/.test(head)&&/填写值/.test(head)){
    const bools={'启用过滤':'on','按薪资范围过滤':'salaryOn','隐藏面议':'hideNegotiable','命中过多时自动恢复':'autoRecover'};
    const nums={'月薪下限':'minMonthly','月薪上限':'maxMonthly','日薪下限':'minDaily'};
    rows.forEach(o=>{
      const k=String(o['项目']||'').trim(), v=String(o['填写值']||'').trim();
      if(!k||!v) return;
      if(bools[k]!==undefined){ cfg[bools[k]]=!/^(否|no|false|0|停用)$/i.test(v); n++; return; }
      if(nums[k]!==undefined){ const num=parseFloat(String(v).replace(/[^0-9.]/g,'')); if(!isNaN(num)){ cfg[nums[k]]=num; n++; } return; }
    });
    return n;
  }
  // ② 三层排除词：按表头名区分
  const wordsMap={'绝对排除词':'words','卡片排除词':'wordsCard','详情排除词':'wordsJd'};
  for(const k in wordsMap){
    if(has(k)){
      rows.forEach(o=>{
        const v=String(o[k]||'').trim(); if(!v) return;
        cfg[wordsMap[k]]=cfg[wordsMap[k]]||[]; if(cfg[wordsMap[k]].indexOf(v)<0) cfg[wordsMap[k]].push(v); n++;
      });
      return n;
    }
  }
  // ③ 公司 / 地点黑名单
  if(has('公司名关键词')){
    rows.forEach(o=>{ const v=String(o['公司名关键词']||'').trim(); if(v&&cfg.blackCompanies.indexOf(v)<0){ cfg.blackCompanies.push(v); n++; } });
    return n;
  }
  if(has('地点')&&has('关键词')){   // 「地点/地址关键词」
    rows.forEach(o=>{
      const key=Object.keys(o).find(x=>/地点|地址/.test(x))||'';
      const v=String(o[key]||'').trim(); if(v&&cfg.blackAreas.indexOf(v)<0){ cfg.blackAreas.push(v); n++; }
    });
    return n;
  }
  // ④ 已投名单
  if(has('公司名/岗位名/职位ID')||has('职位ID')){
    rows.forEach(o=>{
      const key=Object.keys(o).find(x=>/职位ID|公司名|岗位名/.test(x))||'';
      const v=String(o[key]||'').trim(); if(!v) return;
      cfg.applied=cfg.applied||[]; if(cfg.applied.indexOf(v)<0){ cfg.applied.push(v); n++; }
    });
    return n;
  }
  // ⑤ 其余（表格规则：老模板与新模板的同名表都在这一类）
  rows.forEach(o=>{
    ...
    if(/黑名单|blacklist/i.test(type)){
      if(/地点|区域|地区|位置|area|location/i.test(field)||/地点/.test(type)) cfg.blackAreas.push(value);
      else cfg.blackCompanies.push(value);
      n++; return;
    }
    if(/已投|投递|applied/i.test(type)){
      cfg.applied=cfg.applied||[]; cfg.applied.push(value); n++; return;
    }
    const enabled=!/^(否|no|false|0|停用)$/i.test(String(o['启用']||o['enabled']||'是').trim());
    cfg.rules.push({type, field, op:String(o['匹配']||o['op']||'包含').trim(), value,
      action:String(o['动作']||'隐藏').trim(), enabled, note:String(o['备注']||o['note']||'').trim()});
    n++;
  });
  return n;
}
...
function downloadTemplateXlsx(){
  const sheets=[
    {name:'① 说明', rows:[
      ['一表对一框：下面每张表对应面板里的一个框，填完导入即可'],
      ['怎么导入','把某张表连表头一起复制 → 面板点「粘贴导入」；或把那张表另存为 .csv → 点「导入表格文件」'],
      ['注意','导入是「追加」不是覆盖，同一张表别导两次；值里带逗号要用英文引号包住'],
      ['字段从哪来','见「⑩ 字段对照（前端/后端）」页']
    ], widths:[16,80], boldHeader:false, freeze:true},
    {name:'② 基础开关与薪资', rows:[
      ['项目','填写值','说明'],
      ['启用过滤','是','关掉=整个过滤停用'],
      ['按薪资范围过滤','是','不看薪资就填「否」'],
      ['隐藏面议','否','面议岗位是否隐藏'],
      ['命中过多时自动恢复','否','一页被藏太多时自动放宽（默认关）'],
      ['月薪下限','4000','低于这个数隐藏'],
      ['月薪上限','10000','高于这个数隐藏'],
      ['日薪下限','150','日薪岗位低于这个数隐藏']
    ], widths:[20,12,40], boldHeader:true, freeze:true,
     validations:[{range:'B2:B30', list:'是,否'}]},
    {name:'③ 绝对排除词', rows:[
      ['绝对排除词','备注'],
      ['销售','卡片或详情任意位置出现就隐藏']
    ], widths:[26,42], boldHeader:true, freeze:true},
    {name:'④ 卡片排除词', rows:[
      ['卡片排除词','备注'],
      ['外包','只在卡片上出现才算（详情正文里出现不算）']
    ], widths:[26,42], boldHeader:true, freeze:true},
    {name:'⑤ 详情排除词', rows:[
      ['详情排除词','备注'],
      ['培训费','只在详情正文里出现才算']
    ...
    {name:'⑥ 公司黑名单', rows:[
      ['公司名关键词','备注'],
      ['某某劳务','绝对项：公司名里带这串就整家隐藏（写「劳务」也能命中）']
    ], widths:[28,46], boldHeader:true, freeze:true},
    {name:'⑦ 地点黑名单', rows:[
      ['地点/地址关键词','备注'],
      ['龙华','卡片地点或详情「工作地址」带这串就隐藏']
    ], widths:[26,46], boldHeader:true, freeze:true},
    {name:'⑧ 已投名单', rows:[
      ['公司名/岗位名/职位ID','备注'],
      ['某某科技有限公司','整家公司隐藏'],
      ['运营专员','只隐藏这个岗位']
    ], widths:[28,42], boldHeader:true, freeze:true},
    {name:'⑨ 表格规则（高级）', rows:[TABLE_HEAD].concat(TEMPLATE_ROWS), widths:[10,12,8,30,8,8,46], boldHeader:true, freeze:true,
      validations:[
    ...
    {name:'⑩ 字段对照（前端/后端）', rows:[
      ['字段','前端（页面 DOM）','后端（接口 / 字段）','什么时候有值','能否用于规则'],
      ['岗位名','.job-name / [class*="jobName"] / .job-title','Vue jobList[i].jobName、列表接口 jobName','列表页即时','能'],
      ['公司名','.company-name / [class*="brandName"] / .company-info','Vue brandName、列表接口 brandName','列表页即时','能'],
      ['薪资','.salary（乱码时跳过）','列表接口 salaryDesc（明文，优先）','列表页即时','能（小于/大于/等于）'],
      ['标签','卡片标签行 .job-labels','接口 tags / labels','列表页即时','能'],
      ['地点','卡片底行 .job-area','接口地点字段','列表页即时','能'],
      ['卡片全文','卡片 DOM 全文（已剥掉脚本自绘角标）','—','列表页即时','能'],
      ['行业','—','公司接口 brandIndustry','抓到接口后','能'],
      ['规模','—','公司接口 brandScaleName','抓到接口后','能'],
      ['融资','—','公司接口 brandStageName','抓到接口后','能'],
      ['在招职位数','—','监控脚本镜像的 companyJobs','打开过该公司详情后','能（大于/小于）'],
      ['详情正文','详情面板 .job-sec-text 等','/wapi/zpgeek/job/detail.json?securityId=&lid=','点过卡片或补取详情后','能'],
      ['AI判定','—','bw_company_intel（当前没有任何脚本在写）','暂无','暂不能（写了规则也不命中）']
    ], widths:[14,34,36,20,16], boldHeader:true, freeze:true}
  ];
...
function ensureUi(){
  ...
  wrap.innerHTML='<div class="bwf-panel" id="bwfPanel"></div>'+
    ...
    '<button class="bwf-btn" id="bwfReveal">显示被过滤岗位</button>'+
    '<button class="bwf-btn" id="bwfRules">设置</button>'+
  ...
  wrap.querySelector('#bwfReveal').addEventListener('click',()=>{
    ...
    ui.wrap.querySelector('#bwfReveal').textContent=cfg.reveal?'隐藏被过滤岗位':'显示被过滤岗位';
  });
  ...
  wrap.querySelector('#bwfReveal').textContent=cfg.reveal?'隐藏被过滤岗位':'显示被过滤岗位';
...
function syncBanner(){
  try{
    ...
    if(cfg.on&&cfg.reveal) msgs.push('当前是「显示被过滤岗位」模式：命中的岗位不隐藏，只标红框 + 角标（点小条上的「隐藏被过滤岗位」切回隐藏）');
...
function renderPanel(){
  ...
  p.innerHTML=
    ...
    '</div>'+
    '<div class="bwf-pane" data-pane="abs"><div class="bwf-hint">命中即隐藏：岗位名 / 公司 / 标签 / 卡片全文 + 详情正文（详情要取到才算）<br>「隐藏」＝只在你浏览器里藏起来、不发任何请求，随时可「取消隐藏」；这里藏的是<b>岗位卡片</b>，聊天会话的隐藏在「聊天体检」脚本里。</div>'+
    ...
    '<span class="bwf-mute">单条点「取消隐藏」即可</span></div>'+
    '</div></details>'+
  ...
  if(hl){
    ...
    hl.innerHTML=ids.length?ids.slice(0,50).map(id=>{
      ...
      return '<div class="bwf-row" style="margin:2px 0"><span style="flex:1">'+escHtml(m.n||id)+(m.c?(' · '+escHtml(m.c)):'')+'</span>'+
        '<button class="bwf-ghost" data-hide="'+escHtml(id)+'" style="padding:2px 8px">取消隐藏</button></div>';
...
document.addEventListener('contextmenu',(ev)=>{
  try{
    ...
    const items=[];
    if(rec) items.push({label:'取消隐藏（恢复显示）',fn:()=>unhideJob(jobId)});
